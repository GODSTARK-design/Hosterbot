...
try:
    ...
    print("✅ 𝘽𝙊𝙏 𝙏𝙊𝙆𝙀𝙉 𝙄𝙎 𝙑𝘼𝙇𝙄𝘿!")
except telebot.apihelper.ApiException as e:
    print(f"❌ 𝙄𝙉𝙑𝘼𝙇𝙄𝘿 𝘽𝙊𝙏 𝙏𝙊𝙆𝙀𝙉: {e}")
    exit(1)
...
UPLOAD_LIMITS_FILE = "upload_limits.json"

if os.path.exists(UPLOAD_LIMITS_FILE):
    with open(UPLOAD_LIMITS_FILE) as f:
        user_upload_limits = json.load(f)
else:
    user_upload_limits = {}

def save_upload_limits():
    with open(UPLOAD_LIMITS_FILE, "w") as f:
        json.dump(user_upload_limits, f)
...
def start_command(message):   
    ...
    markup.add(types.InlineKeyboardButton("❣️ 𝘾𝙃𝘼𝙉𝙉𝙀𝙇 ⚡", url="https://t.me/SP1DEYYXPR1M3"))
...
def send_file_action_menu(chat_id, filename):
    ...
    # 1st line -> Start & Stop
    markup.add(
        types.InlineKeyboardButton("⚡ 𝙎𝙏𝘼𝙍𝙏", callback_data=f"start_{filename}"),
        types.InlineKeyboardButton("🛑 𝙎𝙏𝙊𝙋", callback_data=f"stop_{filename}")
    )

    # 2nd line -> Restart & Usage
    markup.add(
        types.InlineKeyboardButton("🔄 𝙍𝙀-𝙎𝙏𝘼𝙍𝙏", callback_data=f"restart_{filename}"),
        types.InlineKeyboardButton("📊 𝙐𝙎𝘼𝙂𝙀", callback_data=f"usage_{filename}")
    )

    # 3rd line -> Delete & View Log
    markup.add(
        types.InlineKeyboardButton("🗑️ 𝘿𝙀𝙇𝙀𝙏𝙀", callback_data=f"delete_{filename}"),
        types.InlineKeyboardButton("📄 𝙑𝙄𝙀𝙒 𝙇𝙊𝙂", callback_data=f"log_{filename}")
    )

    bot.send_message(
        chat_id,
        f"📁 𝘼𝘾𝙏𝙄𝙊𝙉 𝙈𝙀𝙉𝙐 𝙁𝙊𝙍 `{filename}`",
        reply_markup=markup,
        parse_mode='Markdown'
    )
...
def handle_document(message):
    ...
    try:
        ...
        return bot.reply_to(message, f"⚠️ 𝘿𝙤𝙬𝙣𝙡𝙤𝙖𝙙 𝙀𝙧𝙧𝙤𝙧: {e}")

    filename = os.path.basename(message.document.file_name)
    safe_filename = re.sub(r'[^\w\.-]', '_', filename)

    if not filename.lower().endswith(".py"):
        return bot.reply_to(message, "🚫 *𝙊𝙉𝙇𝙔 .py 𝙁𝙞𝙡𝙚𝙨 𝘼𝙡𝙡𝙤𝙬𝙚𝙙!*", parse_mode="Markdown")

    # 📂 Step 2: Save in pending folder
    pending_user_dir = os.path.join(PENDING_DIR, str(user_id))
    os.makedirs(pending_user_dir, exist_ok=True)
    pending_path = os.path.join(pending_user_dir, safe_filename)

    with open(pending_path, 'wb') as f:
        f.write(file_data)

    upload_time = datetime.datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S UTC")

    # 📤 Step 3: Send to Owner for Approval
    caption = (
        f"📂 <b>𝙉𝙀𝙒 𝙁𝙄𝙇𝙀 𝙐𝙋𝙇𝙊𝘼𝘿 (Approval Needed)</b>\n\n"
        f"👤 𝙉𝙖𝙢𝙚: <b>{html.escape(message.from_user.first_name)}</b>\n"
        f"🔗 𝙐𝙨𝙚𝙧: @{message.from_user.username or 'None'}\n"
        f"🆔 𝙐𝙨𝙚𝙧 𝙄𝘿: <code>{user_id}</code>\n"
        f"📄 𝙁𝙞𝙡𝙚: <code>{safe_filename}</code>\n"
        f"⏰ 𝙏𝙞𝙢𝙚: {upload_time}\n\n"
        f"⚡ 𝘼𝙘𝙘𝙚𝙥𝙩 𝙤𝙧 𝙍𝙚𝙟𝙚𝙘𝙩?"
    )
    ...
    markup = types.InlineKeyboardMarkup()
    markup.add(
        types.InlineKeyboardButton("✅ 𝘼𝘾𝘾𝙀𝙋𝙏", callback_data="approve_accept_pending"),
        types.InlineKeyboardButton("❌ 𝙍𝙀𝙅𝙀𝘾𝙏", callback_data="approve_reject_pending")
    )

    with open(pending_path, 'rb') as doc:
    ...
    kb.add(
        types.InlineKeyboardButton("✅ 𝘼𝘾𝘾𝙀𝙋𝙏", callback_data=f"approve_accept_{owner_msg_id}"),
        types.InlineKeyboardButton("❌ 𝙍𝙀𝙅𝙀𝘾𝙏", callback_data=f"approve_reject_{owner_msg_id}")
    )
    bot.edit_message_reply_markup(chat_id=OWNER_ID, message_id=owner_msg_id, reply_markup=kb)

    pending_approvals[owner_msg_id] = {
        "user_id": user_id,
    ...
    # 📨 Notify user
    bot.reply_to(message, "⏳ 𝙔𝙤𝙪𝙧 𝙛𝙞𝙡𝙚 𝙝𝙖𝙨 𝙗𝙚𝙚𝙣 𝙨𝙚𝙣𝙩 𝙛𝙤𝙧 𝙤𝙬𝙣𝙚𝙧 𝙖𝙥𝙥𝙧𝙤𝙫𝙖𝙡 ⚡", parse_mode="HTML")
...
def clean_memory(message):
    steps = [
        ...
        "🔍 𝙎𝘾𝘼𝙉𝙉𝙄𝙉𝙂 𝙁𝙄𝙇𝙀𝙎 & 𝘿𝙄𝙍𝙀𝘾𝙏𝙊𝙍𝙄𝙀𝙎...",
        "🗄️ 𝘾𝙇𝙀𝘼𝙍𝙄𝙉𝙂 𝙇𝙊𝙂𝙎, 𝘾𝘼𝘾𝙃𝙀 & 𝙐𝙋𝙇𝙊𝘼𝘿𝙎...",
        "♻️ 𝙊𝙋𝙏𝙄𝙈𝙄𝙕𝙄𝙉𝙂 𝙈𝙀𝙈𝙊𝙍𝙔...",
        "✅ 𝘾𝙊𝙈𝙋𝙇𝙀𝙏𝙄𝙉𝙂 𝘾𝙇𝙀𝘼𝙉𝙐𝙋..."
    ]
    ...
    # Linux-specific disk cache clear (wrapped safely)
    try:
    ...
    # Linux-specific package cleanup (safe)
    try:
...
def list_my_files(message):
    user_dir = os.path.join(UPLOAD_DIR, str(message.chat.id))
    if not os.path.exists(user_dir):
        return bot.reply_to(
            message,
            "📂 <b>𝙉𝙊 𝙁𝙄𝙇𝙀𝙎 𝙐𝙋𝙇𝙊𝘼𝘿𝙀𝘿!</b>\n\n⚡ 𝙐𝙎𝙀 /upload 𝙏𝙊 𝘼𝘿𝘿 𝙉𝙀𝙒 𝙁𝙄𝙇𝙀𝙎.",
            parse_mode="HTML"
        )
    ...
    markup = types.InlineKeyboardMarkup()
    for f in files:
        markup.add(types.InlineKeyboardButton(f"📄 {f}", callback_data=f"filemenu_{f}"))
...
@bot.callback_query_handler(func=lambda call: call.data.startswith("filemenu_"))
def file_menu(call):
    filename = call.data.replace("filemenu_", "")
    
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.add(
        types.InlineKeyboardButton("▶️ 𝙎𝙏𝘼𝙍𝙏", callback_data=f"start_{filename}"),
        types.InlineKeyboardButton("⏹️ 𝙎𝙏𝙊𝙋", callback_data=f"stop_{filename}"),
        types.InlineKeyboardButton("🔄 𝙍𝙀𝙎𝙏𝘼𝙍𝙏", callback_data=f"restart_{filename}"),
        types.InlineKeyboardButton("📜 𝙇𝙊𝙂𝙎", callback_data=f"logs_{filename}"),
        types.InlineKeyboardButton("📊 𝙐𝙎𝘼𝙂𝙀", callback_data=f"usage_{filename}"),
        types.InlineKeyboardButton("ℹ️ 𝙎𝙏𝘼𝙏𝙐𝙎", callback_data=f"status_{filename}"),
        types.InlineKeyboardButton("🗑️ 𝘿𝙀𝙇𝙀𝙏𝙀", callback_data=f"delete_{filename}")
    )

    bot.edit_message_text(
        f"📁 <b>𝘼𝘾𝙏𝙄𝙊𝙉 𝙈𝙀𝙉𝙐 𝙁𝙊𝙍:</b>\n<code>{filename}</code>",
...
@bot.callback_query_handler(func=lambda call: call.data == "back_to_myfiles")
def back_to_myfiles(call):
    list_my_files(call.message)

@bot.callback_query_handler(func=lambda call: call.data.startswith("status_"))
def status_handler(call):
    filename = call.data.replace("status_", "")

    proc_info = processes.get(filename, {})
    ...
    if pid and psutil.pid_exists(pid):
        ...
        status_text = (
            f"📡 <b>𝙋𝙍𝙊𝙅𝙀𝘾𝙏 𝙎𝙏𝘼𝙏𝙐𝙎 𝙁𝙊𝙍:</b> <code>{filename}</code>\n\n"
            f"🔹 <b>Status:</b> 🟢 Running\n"
            f"🔹 <b>PID:</b> <code>{pid}</code>\n"
            f"🔹 <b>Uptime:</b> {uptime_str}\n"
            f"🔹 <b>Last Run:</b> {datetime.datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S UTC')}\n"
            f"🔹 <b>Last Exit Code:</b> {proc_info.get('exit_code') or 'None'}"
        )
    else:
        status_text = (
            f"📡 <b>𝙋𝙍𝙊𝙅𝙀𝘾𝙏 𝙎𝙏𝘼𝙏𝙐𝙎 𝙁𝙊𝙍:</b> <code>{filename}</code>\n\n"
            f"🔹 <b>Status:</b> 🔴 Stopped\n"
            f"🔹 <b>Last Exit Code:</b> {proc_info.get('exit_code') or 'Unknown'}"
        )
...
@owner_only
def set_credit(message):
    parts = message.text.strip().split()
    if len(parts) != 3 or not parts[1].isdigit() or not parts[2].isdigit():
        return bot.reply_to(message, "⚠️ *𝙐𝙎𝘼𝙂𝙀:* `/credit <user_id> <limit>`", parse_mode='Markdown')

    user_id = parts[1]
    limit = int(parts[2])
    user_upload_limits[user_id] = limit
    save_upload_limits()
    ...
    try:
        bot.send_message(
            int(user_id),
            f"🎁 *𝙐𝙋𝙇𝙊𝘼𝘿 𝘾𝙍𝙀𝘿𝙄𝙏 𝙂𝙍𝘼𝙉𝙏𝙀𝘿!*\n\n"
            f"🔓 𝙉𝙀𝙒 𝙐𝙋𝙇𝙊𝘼𝘿 𝙇𝙄𝙈𝙄𝙏 ➤ `{limit}` 𝙁𝙄𝙇𝙀𝙎 ⚡\n"
            "📂 𝙉𝙊𝙒 𝙔𝙊𝙐 𝘾𝘼𝙉 𝙐𝙋𝙇𝙊𝘼𝘿 𝙈𝙊𝙍𝙀 .𝙋𝙔 𝙎𝘾𝙍𝙄𝙋𝙏𝙎!",
            parse_mode="Markdown"
        )
    except Exception as e:
...
def delete_file_handler(call):
    ...
    if os.path.exists(file_path):
        try:
            os.remove(file_path)
            processes.pop(filename, None)  # अगर file चल रही थी तो उसको भी processes से हटा दो
            bot.answer_callback_query(call.id, "🗑️ File Deleted Successfully!")
            bot.edit_message_text(
                f"✅ <b>𝙁𝙄𝙇𝙀 𝘿𝙀𝙇𝙀𝙏𝙀𝘿:</b> <code>{filename}</code>",
                chat_id=call.message.chat.id,
                message_id=call.message.message_id,
                parse_mode="HTML"
            )
        except Exception as e:
            bot.answer_callback_query(call.id, "⚠️ Delete Failed")
            bot.send_message(call.message.chat.id, f"❌ Error deleting {filename}:\n{str(e)}")
    else:
...
@bot.callback_query_handler(func=lambda call: call.data.startswith("approve_accept_") or call.data.startswith("approve_reject_"))
def handle_approval_decision(call):
    parts = call.data.split("_")
    action = parts[1]
    owner_msg_id = int(parts[2])

    pending = pending_approvals.get(owner_msg_id)
    if not pending:
        return bot.answer_callback_query(call.id, "⚠️ 𝙍𝙚𝙦𝙪𝙚𝙨𝙩 𝙀𝙭𝙥𝙞𝙧𝙚𝙙")

    if call.from_user.id != OWNER_ID and call.from_user.id not in ADMINS:
        return bot.answer_callback_query(call.id, "❌ 𝙉𝙤𝙩 𝘼𝙡𝙡𝙤𝙬𝙚𝙙")
    ...
    # ✅ Accept Flow
    if action == "accept":
        user_final_dir = os.path.join(UPLOAD_DIR, str(user_id))
        os.makedirs(user_final_dir, exist_ok=True)
        final_path = os.path.join(user_final_dir, filename)
        ...
        try:
            ...
            bot.send_message(user_id, f"✅ 𝙔𝙤𝙪𝙧 𝙛𝙞𝙡𝙚 <code>{filename}</code> 𝙖𝙘𝙘𝙚𝙥𝙩𝙚𝙙 & 𝙝𝙤𝙨𝙩𝙚𝙙 ⚡", parse_mode="HTML")
            bot.send_message(OWNER_ID, f"✅ 𝙁𝙞𝙡𝙚 <code>{filename}</code> 𝙖𝙘𝙘𝙚𝙥𝙩𝙚𝙙 & 𝙝𝙤𝙨𝙩𝙚𝙙", parse_mode="HTML")

        except Exception as e:
            bot.send_message(OWNER_ID, f"❌ 𝙃𝙤𝙨𝙩𝙞𝙣𝙜 𝙁𝙖𝙞𝙡𝙚𝙙: {e}", parse_mode="HTML")
            bot.send_message(user_id, f"❌ 𝙔𝙤𝙪𝙧 𝙛𝙞𝙡𝙚 𝙖𝙘𝙘𝙚𝙥𝙩𝙚𝙙 𝙗𝙪𝙩 𝙝𝙤𝙨𝙩𝙞𝙣𝙜 𝙛𝙖𝙞𝙡𝙚𝙙", parse_mode="HTML")

        pending_approvals.pop(owner_msg_id, None)
        bot.answer_callback_query(call.id, "✔️ Accepted & Hosted")

    # ❌ Reject Flow
    elif action == "reject":
        prompt = bot.send_message(
            OWNER_ID,
            f"❌ 𝙀𝙣𝙩𝙚𝙧 𝙍𝙚𝙖𝙨𝙤𝙣 𝙛𝙤𝙧 <code>{filename}</code> (or type skip)",
            parse_mode="HTML"
        )
        pending_reason_wait[OWNER_ID] = owner_msg_id
        bot.register_next_step_handler(prompt, process_rejection_reason)
        bot.answer_callback_query(call.id, "Type reason in chat.")

@bot.message_handler(func=lambda m: m.from_user.id in pending_reason_wait)
def process_rejection_reason(message):
    owner_id = message.from_user.id
    owner_msg_id = pending_reason_wait.pop(owner_id)
    pending = pending_approvals.pop(owner_msg_id, None)
    if not pending:
        return bot.send_message(owner_id, "⚠️ Request not found.")
    ...
    if reason.lower() == "skip":
        bot.send_message(user_id, f"❌ 𝙔𝙤𝙪𝙧 𝙛𝙞𝙡𝙚 <code>{filename}</code> 𝙬𝙖𝙨 𝙍𝙀𝙅𝙀𝘾𝙏𝙀𝘿.", parse_mode="HTML")
        bot.send_message(owner_id, f"📩 𝙍𝙚𝙟𝙚𝙘𝙩𝙞𝙤𝙣 𝙙𝙚𝙡𝙞𝙫𝙚𝙧𝙚𝙙 (no reason).", parse_mode="HTML")
    else:
        bot.send_message(user_id, f"❌ 𝙔𝙤𝙪𝙧 𝙛𝙞𝙡𝙚 <code>{filename}</code> 𝙬𝙖𝙨 𝙍𝙀𝙅𝙀𝘾𝙏𝙀𝘿.\nReason: {reason}", parse_mode="HTML")
        bot.send_message(owner_id, f"📩 𝙍𝙚𝙟𝙚𝙘𝙩𝙞𝙤𝙣 𝙙𝙚𝙡𝙞𝙫𝙚𝙧𝙚𝙙 (with reason).", parse_mode="HTML")

    if os.path.exists(pending['pending_path']):
...
def check_commands(message):
    ...
    commands = {
        ...
        '/listfiles': handle_uploaded_files,
...
def usage_file(call):
    filename = call.data.split("usage_", 1)[1]
    chat_id = call.message.chat.id

    filepath = os.path.join(UPLOAD_DIR, str(chat_id), filename)
    log_path = os.path.join(LOGS_DIR, f"{filename}_log.txt")
    ...
    process_status = "🟢 Running" if filename in processes else "🔴 Stopped"

    uptime = "N/A"
    if filename in processes:
        try:
            p = psutil.Process(processes[filename].pid)
            uptime = str(datetime.timedelta(seconds=int(time.time() - p.create_time())))
        except:
            pass
    ...
    usage_text = (
        ...
        f"📁 Fɪʟᴇ: `{filename}`\n"
        f"💾 Sɪᴢᴇ: `{file_size} KB`\n"
        f"⚙️ Sᴛᴀᴛᴜꜱ: {process_status}\n"
        f"⏳ Uᴘᴛɪᴍᴇ: `{uptime}`\n"
        f"📝 Lᴏɢ: `{log_path}`"
    )

    bot.send_message(chat_id, usage_text, parse_mode="Markdown")
    bot.answer_callback_query(call.id, "📊 Usage details sent!")        
 
@bot.callback_query_handler(func=lambda call: call.data.startswith("restart_"))
def restart_file(call):
    filename = call.data.split("restart_", 1)[1]
    chat_id = call.message.chat.id
    ...
    if filename in processes:
        try:
            ...
            del processes[filename]
            bot.answer_callback_query(call.id, "🛑 𝙎𝙏𝙊𝙋𝙋𝙀𝘿 𝙎𝙐𝘾𝘾𝙀𝙎𝙎𝙁𝙐𝙇𝙇𝙔!")
        except:
            bot.answer_callback_query(call.id, "⚠️ 𝙀𝙍𝙍𝙊𝙍 𝙒𝙃𝙄𝙇𝙀 𝙎𝙏𝙊𝙋𝙋𝙄𝙉𝙂!")
    ...
    try:
        ...
        bot.answer_callback_query(call.id, "🔄 𝙍𝙀𝙎𝙏𝘼𝙍𝙏 𝙎𝙐𝘾𝘾𝙀𝙎𝙎𝙁𝙐𝙇𝙇𝙔!")
    except Exception as e:
        bot.answer_callback_query(call.id, f"❌ 𝙍𝙀𝙎𝙏𝘼𝙍𝙏 𝙁𝘼𝙄𝙇𝙀𝘿: {e}")
...
@bot.message_handler(func=lambda message: message.text.startswith('/') and not any(
    message.text.startswith(cmd) for cmd in [
        ...
        '/listfiles', '/listfiles', '/stats', '/uptime', '/feedback', '/ask',
